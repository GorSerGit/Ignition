...
ИСПРАВЛЕНО: collect_facts_smart возвращает UID'ы ролей и предиката
            (не только текст) — нужно для построения trace_graph.
"""
import os
import sys
import json
...
def perceive_text(text: str, memory: AHMemory) -> list[str]:
    """Извлекает seed-узлы из вопроса (через pymorphy3, при неудаче — через БЯМ)."""
    words = re.findall(r'\b[а-яёa-z]+\b', text.lower())
    seed_uids = []
    stopwords = {
        'что', 'как', 'где', 'когда', 'почему', 'зачем', 'кто', 'какой',
        'это', 'этот', 'все', 'она', 'он', 'оно', 'они', 'мы', 'вы', 'ты',
        'не', 'и', 'а', 'в', 'на', 'с', 'по', 'для', 'от', 'к', 'из',
        'является', 'такой', 'чем', 'какие', 'какая', 'какой', 'каково',
    }
...
def collect_facts_smart(seed_uids: list[str], activations: dict,
                       db: AHDatabase, max_facts: int = 30) -> list[dict]:
    """
    🧠 УМНЫЙ СБОР ФАКТОВ.

    Возвращает список фактов вида:
      {
        "uid":           UID факта (N_xxx),
        "predicate_uid": UID предиката (S_xxx),
        "predicate":     текст предиката,
        "roles":         {role_name: text},
        "roles_uids":    {role_name: UID},   ← для trace_graph
        "text_span":     исходный текст факта,
      }
    """
    seed_set = set(seed_uids)
    fact_scores = defaultdict(float)

    # 1. Жёсткий приоритет: факты, где есть seed-узлы
    for uid in seed_uids:
        links = db.get_links_from(uid) + db.get_links_to(uid)
        for l in links:
            if l.e1_type == "P":
                fact_scores[l.e1] += 100.0
            if l.e2_type == "P":
                fact_scores[l.e2] += 100.0

    # 2. Мягкий приоритет: факты с высокой активацией
    for uid, act in activations.items():
        ...
        for l in links:
            if l.e1_type == "P":
                fact_scores[l.e1] += act
            if l.e2_type == "P":
                fact_scores[l.e2] += act
    ...
    for f_uid in top_fact_uids:
        ...
        predicate = "?"
        predicate_uid = None
        roles = {}       # текст для промпта
        roles_uids = {}  # UID для trace_graph

        for pl in pred_links:
            sym_row = db.conn.execute(
                "SELECT r_text FROM symbols WHERE uid = ?", (pl.e2,)
            ).fetchone()
            if not sym_row:
                continue
            try:
                r_text = json.loads(sym_row[0])
                sym_text = r_text[0] if r_text else "?"
            except Exception:
                sym_text = "?"
            ...
            if pl.link_id == "PREDICATE":
                ...
                roles[pl.link_id] = sym_text
                roles_uids[pl.link_id] = pl.e2
...
def main():
    ...
    while True:
        ...
        if not seed_uids:
            print("❌ Не удалось найти концепты в графе.")
            continue
        ...
        if not facts:
            print("❌ В графе не нашлось фактов для ответа.")
            continue
